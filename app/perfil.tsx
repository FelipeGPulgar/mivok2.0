...
export default function PerfilScreen() {
  ...
  const cargarPerfil = useCallback(async () => {
    try {
      // Verificar si está registrado como DJ en AsyncStorage
      const isDJRegistered = await AsyncStorage.getItem('@mivok/is_dj_registered');
  ...
  // Mostrar SIEMPRE el apodo guardado en el perfil (first_name)
  // Fallbacks: email (antes de @) o 'Usuario'
  const displayName =
    (userProfile?.first_name && userProfile.first_name.trim()) ||
      ...
      >
        ...
        <View style={styles.menuSections}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionContent}>
              {menuItems.slice(isDJ ? 5 : 3).map((item, index) => (
                <TouchableOpacity
                  key={index}
                  style={styles.menuItemModern}
                  onPress={item.onPress}
