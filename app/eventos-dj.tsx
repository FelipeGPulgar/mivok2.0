...
export default function EventosDJScreen() {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      await finalizeCompletedEvents(user.id as string, 'dj');

      const data = await listEventsForUser(user.id as string, 'dj');
      setEvents(data);
    } catch (e) {
      console.error('❌ Error cargando eventos DJ:', e);
      setEvents([]);
    } finally {
      setLoading(false);
    }
  ...
  const empty = useMemo(() => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptySub}>Cuando te contraten, tus eventos aparecerán aquí</Text>
    </View>
  ), []);
  ...
  const handleChat = () => {
    // Navegar al centro de mensajes del DJ
    closeOptions();
    router.push('/chats-dj');
  };
            ...
            onPress: () => {
              ...
              router.push({
                ...
                params: {
                  ...
                  revieweeName: 'Cliente', // TODO: Obtener nombre real del cliente
                },
              });
            },
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                console.error('❌ Error eliminando evento desde DJ UI:', e);
        ...
        { text: 'No, mantener evento', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('❌ Error cancelando evento desde DJ UI:', e);
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} activeOpacity={0.9} onPress={() => openOptions(item)}>
            <View style={styles.cardHeader}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>Evento confirmado</Text>
            ...
            <View style={styles.cardBodyRow}>
              ...
              <Text style={styles.metaValue}>{item.hora_inicio && item.hora_fin ? 'Programado' : '—'}</Text>
            </View>
            <View style={styles.cardBodyRow}>
      ...
      <Modal transparent visible={!!selected} animationType="fade" onRequestClose={closeOptions}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSub}>¿Qué te gustaría hacer?</Text>

            <TouchableOpacity style={styles.primaryAction} onPress={handleChat}>
              <Text style={styles.primaryActionText}>Chatear con Cliente</Text>
            </TouchableOpacity>
